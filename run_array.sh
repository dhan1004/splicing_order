...
#SBATCH --array=1-1000%100 # Replace N with number of samples; %100 = max 100 jobs running simultaneously
#SBATCH --nodes=1
#SBATCH --cpus-per-task=16  # Matches THREADS=8 in your pipeline
#SBATCH --mem=48G  # Adjust based on your per-sample memory needs
#SBATCH --time=12:00:00  # Adjust based on typical runtime per sample
#SBATCH --output=out_files/order_genome_match_%A_%a.out
#SBATCH --error=out_files/order_genome_match_%A_%a.err
#SBATCH --partition=batch  # Use your batch partition

# --- Configuration ---
INPUT_LIST_TSV="/users/dhan30/scratch/splicing_order/data/gsm_sra_list_failed.tsv"
THREADS=8
...
# Line number = array task ID + 1 (to skip header)
